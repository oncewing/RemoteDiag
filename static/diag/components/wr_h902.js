...
export default {
  ...
  async _run() {
    ...
    try {
      ...
      const isErr = (r) => !r.response || r.response.toUpperCase().includes('ERROR');
      if (isErr(rLte)) {
        const rFb = await atCmd('AT$$BANDPREF?', 5);
        const bandLine = (rFb.response || '').split(/\r?\n|\r/).map(l => l.trim()).find(l => /^B\d/i.test(l));
        this._setRow('band', 'ok', { html: `LTE: ${bandLine ? bandLine.replace(/\s+/g, ', ') : '없음'}` });
      } else {
        ...
        const nsa = rNsa.response ? (_parseBandPref(rNsa.response, 'n') || '없음') : '—';
        const sa  = rSa.response  ? (_parseBandPref(rSa.response,  'n') || '없음') : '—';
        this._setRow('band', 'ok', { html: `LTE: ${lte}<br>NSA: ${nsa}<br>SA: &nbsp;${sa}` });
      }
    } catch (e) {
