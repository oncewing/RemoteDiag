...
function applyPermissions() {
  const allTabs = ['adb-shell', 'adb-info', 'at', 'logs', 'kmsg', 'guide'];
...
const SECTION_LOADERS = {
  ...
  'sec-dns':      () => _fetchSectionShell('sec-dns', 'cat /var/run/data/dnsmasq.conf.bridge0'),
...
// ── kmsg tab ──────────────────────────────────────────────────────────
async function startKmsg() {
  if (!selectedSerial) { toast('디바이스를 먼저 선택하세요.', true); return; }
  if (kmsgRunning) await stopKmsg();
  const res = await sendCommand({ type: 'kmsg_start', serial: selectedSerial });
  if (res.success) {
    kmsgRunning = true;
    document.getElementById('kmsg-status').textContent = '▶ 실행 중';
  } else {
    toast(res.error || 'kmsg 시작 실패', true);
  }
}

async function stopKmsg() {
  await sendCommand({ type: 'kmsg_stop' });
  kmsgRunning = false;
